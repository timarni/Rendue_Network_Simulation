...
void Network::resize(const size_t& networkSize){
	...
	RNG.uniform_double(values, 1.0, 100.0);
}
...
bool Network::add_link(const size_t& node_1, const size_t& node_2){
  ...
  else if (links.find(node_1)->second == node_2){return true;}
	...
	for (size_t node=0; node < size(); ++node){
		...
		size_t myDegree (RNG.poisson(lamda)); //- links.count(node)); if (myDegree > 0){} would be better but this here doesn't work, overflow, programm does not end to run
			...
			for(size_t i = 0; i < myDegree; ++i){
				...
				while ((newConnectionPoint == node)) { //or (links.find(newConnectionPoint)->second == node)) { //if the link is already established it shouldn't do it again or can there be several links between two nodes?
...
size_t Network::set_values(const std::vector<double>& myValues){
	values.clear();
	values = myValues;
	return size();
}
